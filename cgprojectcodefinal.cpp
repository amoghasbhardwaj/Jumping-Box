...

{
	...
	glColor3f(0.4294,0.7078,0.8216);		//sky blue
...

{
	...
	glColor3f(landmorfer[landcolor+1]+((landmorfer[landcolor+5])*skyindex/NUM_SKY_COLOR),landmorfer[landcolor+6]+((landmorfer[landcolor+3])*skyindex/NUM_SKY_COLOR),landmorfer[landcolor+9]+((landmorfer[landcolor+8])*skyindex/NUM_SKY_COLOR));
...

{
	...
	glClearColor(0,0,0,0);	//Persian blue
	...
	drawText(700,250,"BOX is damaged");		//display trex is hurt
...

		{
			...
			if(cactpos[i]>=110 && cactpos[i]<=120)//if cactus is in range,be careful and so check if you hit a cactus and hurt T_Rex
				...
				{			//because in this case the body will be directly above the cactus
				     ...
				     printf("Oops!! The Box is damaged\n");//T_Rex got hit
...

{
	...
	glutCreateWindow("Jumping Box");		//window tital name
